...
/* These counters belong to a machine instance, not the lifetime of the process. */
static byte BFlag,BCount,ACount,Drawing;
static int UCount;
...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if((NewRAMPages<4)||(NewRAMPages>256))
  ...
  /* Allocation failure must not form RAM/VRAM pointers or start the CPU. */
  if(!RAMData||!VRAM) return(Mode);
